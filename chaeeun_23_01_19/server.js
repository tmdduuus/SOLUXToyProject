...
// });
app.get('/writecommu', checklogin, function(req, res){
    res.render('writecommu.ejs');
});
...
app.post('/addcommu', function(req, res){
    ...
    db.collection('counter').findOne({name : '게시물 개수'}, function(error, result){
        ...
        var tosave = { _id : totalpostnum + 1, editor : req.user._id, title : req.body.title, writingcommu : req.body.writingcommu }
...
// 지원서 작성 페이지
app.get('/writeappli', checklogin, function(req, res){
    res.render('writeappli.ejs');
});
...
app.post('/addappli', function(req, res){
    ...
    var saveappli = { _id : req.user._id, id : req.user.id, name : req.body.name, phone : req.body.phone, birth : req.body.birth, applitext : req.body.applitext }
...
app.get('/myapplication/:identify', checklogin, function(req, res){
    ...
    db.collection('application').findOne({ _id : req.user._id, id : req.params.identify }, function(error, result){
        console.log(result);
        // 어떤이름으로 : 어떤데이터를ejs파일에꽂아주세요
...
app.get('/editappli/:identify', checklogin, function(req, res){

    db.collection('application').findOne({ _id : req.user._id, id : req.params.identify }, function(error, result){
        console.log(result);
        // 어떤이름으로 : 어떤데이터를ejs파일에꽂아주세요
        res.render('editappli.ejs', { data : result });
    });
});
...
app.put('/editappli', function(req, res){
    ...
    var changeappli = { name : req.body.name, phone : req.body.phone, birth : req.body.birth, applitext : req.body.applitext }

    db.collection('application').updateOne({ _id : req.user._id, id : req.user.id }, { $set : changeappli }, function(err, result){
        // 수정완료시 다른 페이지로 이동
        console.log('수정완료');
        res.redirect('/mypage');
    });
...
app.put('/edit', function(req, res){
    // 폼에 담긴 제목 데이터, 날짜 데이터를 가지고
    // db.collection에다가 업데이트
    db.collection('post').updateOne({ _id : parseInt(req.body.id) }, { $set : { title: req.body.title , writingcommu: req.body.writingcommu }}, function(err, result){
        // 수정완료시 다른 페이지로 이동
        console.log('수정완료');
        res.redirect('/list');
    });
